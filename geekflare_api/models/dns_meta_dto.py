...
class DnsMetaDto(BaseModel):
    """
    DnsMetaDto
    """ # noqa: E501
    url: StrictStr = Field(description="The target URL that was scraped", json_schema_extra={"examples": ["https://example.com"]})
    types: StrictStr = Field(description="List of DNS record types to query. If omitted, all supported types will be returned.", json_schema_extra={"examples": [["A", "MX"]]})
    test: TestMetaDto = Field(description="Test details object")
    __properties: ClassVar[List[str]] = ["url", "types", "test"]

    @field_validator('types')
    def types_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['A', 'AAAA', 'CNAME', 'MX', 'CAA', 'NS', 'SOA', 'SRV', 'TXT']):
            raise ValueError("must be one of enum values ('A', 'AAAA', 'CNAME', 'MX', 'CAA', 'NS', 'SOA', 'SRV', 'TXT')")
        return value
